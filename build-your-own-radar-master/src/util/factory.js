  ...
  blips.forEach((blip) => {
    ...
    if (currentQuadrant && ring) {
      const blipObj = new Blip(
        ...
        blip.isNew.toLowerCase() === 'true',
        blip.isStrategicDirection.toLowerCase() === 'true',
        blip.isUsedByChildCompany1.toLowerCase() === 'true',
        blip.isUsedByChildCompany2.toLowerCase() === 'true',
        blip.isUsedByChildCompany3.toLowerCase() === 'true',
        blip.status,
        blip.topic,
        blip.description,
      )
...
const Factory = function () {
  ...
  self.build = function () {
    const defaultSheetId = '1R8nj0SXWWmkMaLg0iHt6K0RuTsbUZ5TvMmZwkQkDAyk'
    const defaultSheetName = 'Sheet1'
    
    // Automatically load the Google Sheet
    const sheet = new GoogleSheet(defaultSheetId, defaultSheetName)
    sheet.init().build()
  }
...
function setDocumentTitle() {
  document.title = 'Tech Radar Constellation'
}
...
function plotLoading(content) {
  if (!featureToggles.UIRefresh2022) {
    ...
    var bannerText =
      '<h1>Loading your Tech Radar Constellation...</h1><p>Your Technology Landscape will be available in just a few seconds</p>'
...
function plotErrorMessage(exception, fileType) {
  if (featureToggles.UIRefresh2022) {
    ...
    const bannerText =
      '<div><h1>Tech Radar Constellation</h1><p>Once you\'ve <a href ="https://www.thoughtworks.com/radar/byor">created your Radar</a>, you can use this service' +
...
function plotUnauthorizedErrorMessage() {
  ...
  if (!featureToggles.UIRefresh2022) {
    ...
    const bannerText = '<div><h1>Tech Radar Constellation</h1></div>'
